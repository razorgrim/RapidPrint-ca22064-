...
if (isset($_GET['UserID'])) {
    ...
    // Insert the invoice into the database (if it's not already inserted)
    $queryInvoice = "INSERT INTO invoices (UserID, TotalAmount) VALUES (:userId, :totalAmount)";
    $stmtInvoice = $conn->prepare($queryInvoice);
    $stmtInvoice->bindParam(':userId', $userId, PDO::PARAM_INT);
    $stmtInvoice->bindParam(':totalAmount', $totalAmount, PDO::PARAM_STR);
    $stmtInvoice->execute();
} else {
    ...
    <div class="card">
        ...
        <div class="card-body">
            <?php if ($user): ?>
                ...
                <h5 class="text-end">Total Amount: RM<?= number_format($totalAmount, 2) ?></h5>

                <!-- Print Invoice Button -->
                <a href="print_invoice.php?UserID=<?= $userId ?>" class="btn btn-primary mt-4">Update Invoice</a>
