...
try {
    $stmt = $conn->prepare("
        SELECT u.Name, u.Email, a.DateOfBirth, a.Gender, a.PhoneNumber, a.Address, a.ProfilePicture
        FROM user u
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['update_profile'])) {
    ...
    try {
        $update_stmt = $conn->prepare("
            UPDATE user u
            LEFT JOIN admin a ON u.UserID = a.UserID
            SET u.Name = ?, u.Email = ?, a.DateOfBirth = ?, a.Gender = ?, a.PhoneNumber = ?, a.Address = ?, a.ProfilePicture = ?
            WHERE u.UserID = ?
        ");
        $update_stmt->execute([$name, $email, $date_of_birth, $gender, $phone_number, $address, $profile_picture, $user_id]);
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['delete_profile'])) {
    try {
        ...
        $delete_admin = $conn->prepare("DELETE FROM admin WHERE UserID = ?");
...
<div class="profile-container">
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <button type="button" class="back-btn" onclick="window.location.href='admin_profile.php'">Back to Profile</button>
    </form>
